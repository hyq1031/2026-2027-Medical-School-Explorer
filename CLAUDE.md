...
- `opens`/`opensText` (added 2026-07-27, alongside the pre-existing `closes`/`closesText`) — free-text application-open dates, parsed for sorting via `parseAppDate()`/`formatAppDate()` (near `formatDate()`). Unparseable/`"TBC"` strings sort last. Both fields exist on all 24 entries now; keep them in sync when correcting a `closes` date.
- `officialUrl` course-code segments drift and 404 more often than expected (universities restructure program codes between cycles) — **verify the replacement code against a live fetch before writing it in, don't infer/guess it.** Failure that proved this: a 2026-07-27 sweeper pass guessed Tasmania's dead `M3N` code had become `H7X`; a 2026-08-12 re-check found `H7X` also 404s — the real code was `H3X`. A wrong guess here is worse than leaving the stale URL with a note, since it *looks* fixed.

## AU/NZ Country separation (added 2026-08-13)
A top-level Country toggle (🇦🇺 Australia / 🇳🇿 New Zealand / All, default All) sits between the hero and the rest of the page (`#country-toggle`, JS var `currentCountry`). It scopes the grid, Region/State filter, search, sort, and the Application Timeline section to one country — AU and NZ schools are never interleaved. In "All" mode, both the grid and the timeline render as two independently-sorted, clearly labeled sections (Australia then New Zealand) rather than one flat/merged list.

This means grid and timeline rendering each have **two code paths that must be kept in sync** when editing either: `renderGrid()` dispatches to either flat `renderUnis()` (single-country mode, mounts into `#uni-grid`) or grouped `renderGroupedUnis()` (All mode, mounts into `#uni-grid-grouped`, reuses `buildUniCardEl()` for both). Timeline similarly: `renderTimeline()` dispatches to `renderTimelineForCountry()` either once (single-country) or twice (All mode, two `.timeline-country-group` sections). If you add a new card field or timeline event shape, update the shared builder function (`buildUniCardEl`/`renderTimelineForCountry`), not a per-path copy — there's only one of each by design, but it's easy to accidentally special-case a branch.

The timeline deliberately does **not** re-render on every `applyFilters()` call (search keystrokes, ATAR/UCAT slider drags) — only on `init()`, the country-toggle click, and Reset. It's a reference widget, not a "your matches" view; don't wire it into the high-frequency filter path without a reason.
